...
class EditorialRevisionResult(BaseModel):
    ...
    def _result_is_atomic_and_hash_bound(self) -> EditorialRevisionResult:
        ...
        if set(evaluated) | set(unevaluated) != set(targets):
            raise ValueError("evaluated and unevaluated claims must partition targets")
        if set(evaluated) & set(unevaluated):
            raise ValueError("a target cannot be both evaluated and unevaluated")
...
async def revise_audited_draft(
    ...
    *,
    ...
    missing_blocks = tuple(sorted(set(targets) - set(block_by_id)))
    if missing_blocks:
        raise ValueError(
    ...
    target_ids = tuple(
        claim.claim.claim_id
        for block_id in sorted(targets, key=lambda key: block_by_id[key].ordinal)
    ...
    unevaluated_ids = tuple(
        claim_id for claim_id in target_ids if claim_id not in evaluated_set
    )
    complete = not unevaluated_ids and len(evaluated_ids) == len(target_ids)
    if complete:
