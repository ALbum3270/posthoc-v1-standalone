...
from v1.ontology import all_filled, coverage_pct, get_empty_slots
...
from v1.reporter import build_report


MAX_ROUNDS_DEFAULT = 10
SEARCH_RESULTS_PER_ROUND = 3   # 每轮取前 N 条搜索结果
MAX_CHARS_PER_PAGE = 2000      # 每页文本截断长度
...
async def run(topic: str, max_rounds: int = MAX_ROUNDS_DEFAULT) -> None:
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        slot, query = await pick_next_slot_and_query(topic, filled_ids)
        print(f"   🎯 目标槽位: [{slot['dimension']}] {slot['label']}")
        print(f"   🔎 搜索词  : {query}")

        # Step 2: 搜索
        results = await tavily_search(query, max_results=SEARCH_RESULTS_PER_ROUND,
                                      max_chars=MAX_CHARS_PER_PAGE)
        print(f"   📄 搜索结果: {len(results)} 条")

        # Step 3: 抽取 + 写入（取前 2 条页面）
        slot_written = False
        for res in results[:2]:
            ...
            filled_ids.add(slot["slot_id"])
            slot_written = True
            break  # 该槽位已写入，进入下一轮

        if not slot_written:
            print(f"   ⚠️  本轮未能填充 [{slot['label']}]，将在后续轮次重试")
    ...
    print(f"📊 研究结束 — 覆盖率: {coverage_pct(filled_ids):.0f}% "
          f"({len(filled_ids)}/{len(get_empty_slots(set()))+ len(filled_ids)} 槽位)")
    print(f"{'='*60}\n")
    print("生成报告中...\n")

    report = await build_report(gdb, group_id=research_id, topic=topic,
